...
void LCD_Clear(u16 Color)
{
	...
	for(index = 0; index < 76800; index++)//24*16 320*240=76800
	{
		LCD_WriteRAM(Color);    
	}
}
...
/************用于更新数据*****************/
//参数：line 0~9 Column 0~19 
void LCD_Update(u8 Line, u16 Column, float data)
{
	u32 i = 0;
	u8 string[20];
	u16 refcolumn = 319-Column*16;//319;
	sprintf(string,"%.2f",data);//这里改数据宽度
	while (string[i]!= 0)	 //	20
	{
		LCD_DisplayChar(Line, refcolumn, string[i]);
		refcolumn -= 16;
		i++;
	}
}
...
* Output         : None
* Return         : None
LCD_SetDisplayWindow((24*y0)-1, (16*x0)-1, 24*y, 16*x);
					//X:20-x0~20-x0+x Y:y0-y~y0
*******************************************************************************/
